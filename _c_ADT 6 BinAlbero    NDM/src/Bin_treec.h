...
class Bin_treec : 										public Bin_tree<T, int>{
  ...
  public:
    ...
    bool sx_empty(Nodo n) const{   	  	return (spazio[n].sinistro == NIL);};
    bool dx_empty(Nodo n) const{	  	return (spazio[n].destro == NIL);};
    ...
    void write(Nodo n, value_type a){
...
void Bin_treec<T>::ins_sx(Nodo n){
	...
	if (spazio[n].sinistro != NIL)				throw NodeExists();
